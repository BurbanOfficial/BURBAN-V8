...
async function editClient(clientId) {
    try {
        const { doc, getDoc, collection, getDocs, query, where } = window.firebaseModules;
        const db = window.firebaseDb;
        ...
        const addressesList = document.getElementById('clientAddressesList');
        if (client.addresses && client.addresses.length > 0) {
            addressesList.innerHTML = client.addresses.map((addr, index) => `
                <div style="padding: 12px; border: 1px solid var(--border); margin-bottom: 8px; border-radius: 4px;">
                    <strong>${addr.name || 'Adresse ' + (index + 1)}</strong><br>
            ...
            addressesList.innerHTML = '<p style="color: var(--gray);">Aucune adresse enregistrée</p>';
        }
        ...
        if (client.orders && client.orders.length > 0) {
            const ordersPromises = client.orders.map(orderNum => 
                getDoc(doc(db, 'orders', orderNum))
            );
            const ordersDocs = await Promise.all(ordersPromises);
            
            ordersList.innerHTML = ordersDocs.map(orderDoc => {
            ...
            ordersList.innerHTML = '<p style="color: var(--gray);">Aucune commande</p>';
        }
